...
import { Pencil, Trash2, Plus, Target, ImageOff, LogOutIcon, Settings, User, ChevronDown, User2Icon, CircleUser } from "lucide-react";
		...
		<section className="min-h-screen py-10 md:py-14 lg:py-18" aria-label="Sustainable Development Goals">
			<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				...
				<div className="sdg-header mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
					...
					{/* Stats, Add Button, and Dropdown Row */}
					<div className="flex flex-col items-end gap-3">
						<div className="flex items-center gap-3">
							...
							<PermissionGuard permission="create-sdg" fallback={null}>
								<Link
									as="button"
									className="add-goal-btn sdg-btn-shimmer inline-flex items-center rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground transition-all duration-200
                            active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2"
							...
							{/* User Dropdown - Now to the right of the button */}
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									...
									>
										...
										>
											<CircleUser size={24} color="oklch(0.32 0.09 163)" absoluteStrokeWidth />
										</Button>
									</motion.div>
								</DropdownMenuTrigger>
								<DropdownMenuPortal>
									<DropdownMenuContent
										align="end"
										className="w-56 rounded-xl border-border bg-card shadow-xl p-1 animate-in fade-in-0 zoom-in-95 duration-100"
										sideOffset={8}
									>
										{/* User Info Section */}
										...
										<div className="py-1">
											<DropdownMenuItem
												...
												className="gap-3 px-3 py-2 text-sm text-accent rounded-lg cursor-pointer transition-colors
                                           hover:bg-accent/10 focus:bg-accent/10 group"
											>
												<LogOutIcon className="h-4 w-4 group-hover:rotate-12 transition-transform inline me-2" />
												<span>Sign Out</span>
											</DropdownMenuItem>
										</div>
									</DropdownMenuContent>
