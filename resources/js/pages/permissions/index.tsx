...
export default function Index({ permissions }: IndexProps) {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this product?')) {
            const deleteUrl = PermissionController.destroy(id).url;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (mode === 'edit' && selectedCategory) {
            ...
            put(PermissionController.update(selectedCategory.id).url, {
                forceFormData: true,
                onSuccess: (response: { props: FlashProps }) => {
                    toast.success(response.props.flash?.success || 'Goal updated successfully.');
                    closeModal();
                },
                onError: (error: Record<string, string>) => {
            ...
            post(PermissionController.store().url, {
                onSuccess: (response: { props: FlashProps }) => {
                    toast.success(response.props.flash?.success || 'Goal created successfully.');
                    closeModal();
                },
                onError: (error: Record<string, string>) => {
                    toast.error(error?.message || 'Failed to delete Goal.');
